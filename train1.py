...
def train(args):
    ...
    model = Text2Mel()
    model.initialize(feeder.enc_input, feeder.dec_input, feeder.mel_target)
    ...
    with tf.Session() as sess:
        try:
            ...
            while not coord.should_stop():
                step, loss, _ = sess.run([model.global_step, model.loss, model.optimize])
                print('Step: {}, Loss: {:.5f}'.format(step, loss))

                if step % checkpoint_step == 0:
                    saver.save(sess, checkpoint_path, global_step=step)
                    input_seq, alignment, pred, target = sess.run([
                        model.enc_input[0], model.alignment[0], model.mel_output[0], model.mel_target[0]])
