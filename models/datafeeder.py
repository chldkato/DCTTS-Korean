...
class DataFeeder(threading.Thread):
    def __init__(self, coordinator, mode):
        ...
        if self.mode == 1:
            ...
            queue = tf.FIFOQueue(8, [tf.int32, tf.float32, tf.float32])
            self.enqueue = queue.enqueue(self.placeholder)
            self.enc_input, self.dec_input, self.mel_target = queue.dequeue()
            self.enc_input.set_shape(self.placeholder[0].shape)
            self.dec_input.set_shape(self.placeholder[1].shape)
            self.mel_target.set_shape(self.placeholder[2].shape)
